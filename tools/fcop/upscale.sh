...
# (read silently, never stored). Wraps tools/fcop/upscaleTextures.py.
#
# Usage:  bash tools/fcop/upscale.sh
set -uo pipefail   # no -e: a failed action should return to the menu, not exit

HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO="$(cd "$HERE/../.." && pwd)"
MODELS="$REPO/packages/client/public/models"
VENV="$REPO/tools/fcop/_local/venv"
TOOL="$REPO/tools/fcop/upscaleTextures.py"
PY=""
...
while true; do
  select method in "Lanczos (faithful baseline)" "Real-ESRGAN (sharp, pip)" \
                   "Gemini 2.5 Flash Image" "Compare variants" "Exit"; do
    case "${method:-}" in
