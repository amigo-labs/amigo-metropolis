...
export interface LocalInputSource {
  ...
  isConnected(): boolean;
  /**
   * Refresh aim for pointer devices (projects the cursor onto the avatar's
   * ground plane within this player's viewport). No-op for stick aim. Call
   * once per tick before sample(). Allocation-free.
   */
  updateAim(
    camera: THREE.Camera,
    avatarX: number,
