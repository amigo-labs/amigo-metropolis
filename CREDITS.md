...
| Sound effects — Precinct Assault cues | `packages/client/src/audio/presets.ts` | sfxr presets for the alarm / pickup / production / core-hit events (rules.md §9); these are authored, not extracted. `alarm` is authored permanently: the original's alert cue was wired in the undecoded `Cfun` mission script, not in any sound table, so there is nothing to extract (`docs/specs/fcop-logic.md` §8.6) |
...
| `packages/client/public/models/units/` (units) + raws in `tools/generators/units/raw/fcop/` | *Future Cop: L.A.P.D.* Precinct Assault container **Mp** | original Cobj unit models (X1-Alpha hover form 16, Hovertank 30, Flyer 41, heavy gunship 36, Sky Captain jet 54 / gunship form 57, outpost flag console 29), extracted as glb by `extract_objects.py` in `amigo-labs/fcop-reverse-engineering`, then processed by `bun run gen:units` (footprint/origin/orientation, texture pages packed, team units desaturated for the instanceColor tint) |
